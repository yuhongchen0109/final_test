...
def main():
    ...
    '',
    ['About Us','Contact Us','Top Up','Historical Figures Information','Product','History','Movie Recommendation List'])
        # 初始化資料庫
        ...
        if option == 'About Us':
         ...
         st.markdown('''Here is an introduction to famous historical figures from different countries. You can ***_:blue[click the button]_*** ***_:red[according to the person you want to know about for a deeper understanding]_***.''')
        ...
        """
         ...
         with tab3:
          ...
          st.divider()
          st.write('若想了解更多資訊，請根據以下網址輸入至搜尋內容中。https://youtu.be/smLEpN_hRn0')
          youtube_url = st.text_input("搜尋影片")
          ...
          if youtube_url:
           ...
           st.success('Search successfully')             
    ...
    ['Nazi Germany','Empire of Japan','Soviet Union'])
         ...
         if option1 == 'Nazi Germany':
          ...
          tab1, tab2, tab3, tab4 = st.tabs(["Pre War", "WWII", "End War","Holocaust"])

          with tab1:
           st.markdown('**_Pre_ War**')
           st.audio("Das Deutschlandlied  Germany National Anthem [FIRS.MP3", format='audio/mpeg', loop=True)
          ...
          with tab2:
           st.markdown('**_WWII_**')
           st.audio("FSO 2016 Oficial _ Schindler s List(1).MP3", format='audio/mpeg', loop=True)           
          ...
          with tab4:
           st.audio("Schindler s list - John Williams - NL orchestra(1).MP3", format='audio/mpeg', loop=True)
           st.header("納粹大屠殺")
           st.markdown("二戰前全世界有1500萬猶太人，而歐洲總共有近900萬猶太人，其中***_:red[約600萬猶太人近三分之二被害]_***，包括近150萬兒童。一些學者稱大屠殺亦當涵蓋近500萬非猶太遇難者，由此總受害人數將達到近1,100萬人。屠殺發生於納粹德國、德佔歐洲地區及德國盟國所控區域。1941年至1945年，納粹對歐洲諸種族及政治群體展開迫害，猶太人遭到廣泛系統性屠殺，其規模為史上最大") 
           with st.expander("The beginning of the persecution"):   
            st.markdown('***1938年水晶之夜行動***')                     
            video_file = open('The Girl In The Red Coat _ Schindler s List (1994) _ Screen Bites.mp4', 'rb')
            video_bytes = video_file.read()
            st.video(video_bytes)

           st.write("德國與德國占領區有***_:red[近42,500個設施用於集中關押受害者]_***，將其作為奴隸勞工使用，對其進行屠殺或其他反人權活動。參與執行大屠殺的總人數估計超過200,000人。")
           col1,col2 = st.columns(2)    
           with col1:       
            image = Image.open('auschwitz.jpg')
            st.image (image, caption='門牌標示「勞動換取自由」字樣', use_column_width=True)
            st.markdown("奧斯維辛集中營於1940年4月27日，由納粹德國親衛隊領導人希姆萊下令建造。1942年1月20日舉行的***_:red[萬湖會議通過「猶太人最終解決方案」]_***，透過滅絕營實行有系統的猶太人大屠殺行動，***_:red[估計約有110萬人在奧斯維辛被殺]_***。")
           with col2:
            image = Image.open('camp people.webp')
            st.image (image, caption='集中營生還者', use_column_width=True)
           st.divider() 
           with st.expander("The beginning of the persecution"):   
            st.markdown('***猶太大屠殺***')                                 
            video_file = open('Schindler s List (4_9) Movie CLIP - Bach or Mozart.mp4', 'rb')
            video_bytes = video_file.read()
            st.video(video_bytes)

           st.markdown("迫害與屠殺分階段進行，最終發展為***_:red[「猶太人問題的最終解決方案」]_***，對歐洲猶太人進行集體滅絕。最初德國政府通過法案（如《1935年紐倫堡法案》）以將猶太人自社會中排除出去。1933年起納粹開始建立一系列集中營，而在1939年第二次世界大戰爆發後開始建立猶太區。1941年，德國佔領蘇聯在東歐的大片領土，開始廣泛獲取新領土，別動隊在這些區域屠殺了近兩百萬猶太人、游擊隊員及其他群體，手段普遍為大規模射殺。至1942年年末，受害者普遍乘火車前往滅絕營，若能在旅途中倖存，則將於毒氣室中遭系統性殺害。這一狀況一直持續至1945年4月至5月歐洲戰場尾聲階段。")           
           image = Image.open('dead people.jfif')
           st.image (image, caption='放置於集裝車的屍體', use_column_width=True) 
           ...
           st.audio("日本のいちばん長い日（「日本のいちばん長い日」オリジナル・サウンドトラック）富貴晴美.mp3", format='audio/mpeg', loop=True)           
           ...
           if st.button('Play'):
            ...
            if values ==(1917,1945):
             st.audio("The Red Army Choir Alexandrov - The Sacred War (Sv.MP3", format='audio/mpeg', loop=True)             
            ...
            if values ==(1980,1991):
             with st.expander("Disintegration of the soviet union"):
              ...
              video_bytes = video_file.read()
              st.video(video_bytes)
        elif option == 'Movie Recommendation List': 
          st.header("Movie Recommendation List")
          st.info("This is a recommended list page about historical movies. If you want to watch the trailer, please click the button.")
          tab1, tab2, tab3 = st.tabs(["Nazi Germany", "Empire of Japan", "Soviet Union"]) 
          def open_url_in_new_tab(url):
           script = f"""
            <script type="text/javascript">
        ...
        """
          ...
          with tab1:
           col1,_,col3=st.columns(3)
           with col1:
            image = Image.open('downfall.jpg')
            st.image (image, caption='帝國毀滅(Der Untergang/ Downfall)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("帝國毀滅"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=2XiY1ldtWVU")

            image = Image.open('unser mutter unser vater.jpg')
            st.image (image, caption='我們的父輩(Unsere Mütter, unsere Väter)', use_column_width=True) 
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("我們的父輩"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=TmyGPX23px4&pp=ygUkdW5zZXJlIG3DvHR0ZXIgdW5zZXJlIHbDpHRlciB0cmFpbGVy") 

            image = Image.open("schindler's list.jpg")         
            st.image (image, caption="辛德勒的名單(Schindler's list)", use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("辛德勒的名單"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=gG22XNhtnoY")   
             
            image = Image.open('der hauptmann.jpg')
            st.image (image, caption="冒牌上尉(The Captain/ Der Hauptmann)", use_column_width=True)  
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("冒牌上尉"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=D_zSFeHfglc")

           with col3:
            image = Image.open('the rise of evil.jpg')
            st.image (image, caption='惡魔崛起(The Rise of Evil)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("惡魔崛起"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=_fedeRzvPV4")

            image = Image.open('persian lessons.jpg')
            st.image (image, caption='波斯語課(Persian Lessons)', use_column_width=True) 
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("波斯語課"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=Q6zQuPLd9HQ")

            image = Image.open("die wannseekoferenz.jpg")
            st.image (image, caption="萬湖會議/淨化論(Die Wannseekoferenz)", use_column_width=True) 
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:") 
            if st.button("萬湖會議/淨化論"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=UgGBBllL9Gs")

          with tab2:
           col1,_,col3=st.columns(3)
           with col1:
            image = Image.open('JAPAN LONG DAY.jpg')
            st.image (image, caption='日本最長的一天(日本のいちばん長い日)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("日本最長的一天"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=LxEWiHiiCkk")
            
            image = Image.open('ISLAND.jpg')
            st.image (image, caption='硫磺島的來信(硫黄島からの手紙)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("硫磺島的來信"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=0x54bOTdJA0")     

            image = Image.open('MEN YAMAMOTO.jpg')
            st.image (image, caption='男人們的大和(男たちの大和)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("男人們的大和"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=Fggvp2WcmVw")                        

           with col3:
            image = Image.open('ATTACK ON PEARL HARBOR.jpg')
            st.image (image, caption='山本五十六(聯合艦隊司令長官 山本五十六 – 太平洋戦争70年目の真実)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("山本五十六"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=vWT2ydFM8us")

            image = Image.open('PACIFIC.jpg')
            st.image (image, caption='太平洋的奇蹟(太平洋の奇跡)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("太平洋的奇蹟"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=V2TLrPESN5k")

          with tab3:
           col1,_,col3=st.columns(3)
           with col1:
            image = Image.open('ENEMY AT THE GATES.jpg')
            st.image (image, caption='大敵當前(Enemy at the Gates)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("大敵當前"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=4O-sMh_DO6I")
            
            image = Image.open('FRONT.jpg')
            st.image (image, caption='致命最前線(Подольские курсанты)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star:")
            if st.button("致命最前線"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=enIkbBS_AAo")     

            image = Image.open('BERLIN WOMEN.jpg')
            st.image (image, caption='柏林的女人(A Woman In Berlin)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("柏林的女人"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=-xF8f8xti6o")                        

           with col3:
            image = Image.open('RED OCTOBER.jpg')
            st.image (image, caption='獵殺紅色十月號(The Hunt for Red October)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star:")
            if st.button("獵殺紅色十月號"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=NSiAsQMBSRA")

            image = Image.open('STALIN DEATH.jpg')
            st.image (image, caption='史達林死了沒?(The Death of Stalin)', use_column_width=True)
            # 顯示五顆星星
            st.markdown("***:blue[Rating:]***:star::star::star::star::star:")
            if st.button("史達林死了沒?"):
             open_url_in_new_tab("https://www.youtube.com/watch?v=7KWg2nTYmk8")
